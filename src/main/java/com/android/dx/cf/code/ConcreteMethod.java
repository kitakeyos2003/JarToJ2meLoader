 ...
 * It implements {@link Method}, so it provides all the original access (by
 * delegation), but it also constructs and keeps useful versions of stuff
 * extracted from the method's {@code Code} attribute.
 */
public final class ConcreteMethod implements Method {
     ...
     * @param keepLines whether to keep the line number information (if any)
     * @param keepLocals whether to keep the local variable information (if any)
     */
    ...
    public ConcreteMethod(Method method, CstString sourceFile,
        ...
        if (keepLines) {
            for (AttLineNumberTable lnt = (AttLineNumberTable) codeAttribs.findFirst(AttLineNumberTable.ATTRIBUTE_NAME);
                    lnt != null;
        ...
        if (keepLocals) {
             ...
             */
            for (AttLocalVariableTable lvt = (AttLocalVariableTable) codeAttribs.findFirst(
            ...
            for (AttLocalVariableTypeTable lvtt = (AttLocalVariableTypeTable) codeAttribs.findFirst(
    ...
    /**
     * Returns a {@link SourcePosition} instance corresponding to the given
     * bytecode offset.
     *
