...
public abstract class BaseMachine implements Machine {
    ...
    /**
     * {@code >= -1;} count of the results, or {@code -1} if no results have
     * been set
     */
     ...
     *
     * @return {@code >= -1;} the salient local index or {@code -1} if none was
     * set since the last time {@link #clearArgs} was called
     */
    ...
    /**
     * Gets the target local register spec of the current operation, if any. The
     * local target spec is the combination of the values indicated by a
     * previous call to {@link #localTarget} with the type of what should be the
     * sole result set by a call to {@link #setResult} (or the combination
     * {@link #clearResult} then {@link #addResult}.
     *
     * @param isMove {@code true} if the operation being performed on the local
     * is a move. This will cause constant values to be propagated to the
     * returned local
     * @return {@code null-ok;} the salient register spec or {@code null} if no
     * local target was set since the last time {@link #clearArgs} was called
     */
     ...
     *
     * <p>
     * <b>Note:</b> If there is more than one result value, the others may be
     * added by using {@link #addResult}.</p>
     *
    ...
    /**
     * Gets the count of results. This throws an exception if results were never
     * set. (Explicitly clearing the results counts as setting them.)
     *
    ...
    /**
     * Stores the results of the latest operation into the given frame. If there
     * is a local target (see {@link #localTarget}), then the sole result is
     * stored to that target; otherwise any results are pushed onto the stack.
     *
