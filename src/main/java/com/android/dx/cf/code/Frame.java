...
/**
 * Representation of a Java method execution frame. A frame consists of a set of
 * locals and a value stack, and it can be told to act on them to load and store
 * values between them and an "arguments / results" area.
 */
public final class Frame {
    ...
    /**
     * {@code null-ok;} stack of labels of subroutines that this block is nested
     * in
     */
     ...
     * all-uninitialized values (represented as {@code null}s) and the stack
     * starts out empty.
     *
    ...
    /**
     * Makes and returns a mutable copy of this instance. The copy contains
     * copies of the locals and stack (that is, it doesn't share them with the
     * original).
     *
    ...
    /**
     * Replaces all the occurrences of the given uninitialized type in this
     * frame with its initialized equivalent.
     *
    ...
    /**
     * Returns the largest subroutine nesting this block may be in. An empty
     * list is returned if this block is not in any subroutine. Subroutines are
     * identified by the label of their start block. The list is ordered such
     * that the deepest nesting (the actual subroutine this block is in) is the
    ...
    /**
     * Returns a Frame instance representing the frame state that should be used
     * when returning from a subroutine. The stack state of all subroutine
     * invocations is identical, but the locals state may differ.
     *
     * @param startLabel {@code >=0;} The label of the returning subroutine's
     * start block
     * @param subLabel {@code >=0;} A calling label of a subroutine
     * @return {@code null-ok;} an appropriatly-constructed instance, or null if
    ...
    /**
     * Merges two frames. If the merged result is the same as this frame, then
    ...
    /**
     * Merges this frame's subroutine lists with another. The result is the
     * deepest common nesting (effectively, the common prefix of the two lists).
     *
    ...
    /**
     * Adjusts a locals array to account for a merged subroutines list. If a
     * frame merge results in, effectively, a subroutine return through a throw
     * then the current locals will be a LocalsArraySet that will need to be
     * trimmed of all OneLocalsArray elements that relevent to the subroutine
     * that is returning.
     *
     * @param locals {@code non-null;} LocalsArray from before a merge
     * @param subroutines {@code non-null;} a label list of subroutine start
     * blocks representing the subroutine nesting of the block being merged
     * into.
     ...
     * {@code predLabel}. Only called on the frame at the first block of a
     * subroutine.
     *
     * @param other {@code non-null;} another frame
    ...
    /**
     * Makes a frame for a subroutine start block, given that this is the ending
     * frame of one of the subroutine's calling blocks. Subroutine calls may be
     * nested and thus may have nested locals state, so we start with an initial
     * state as seen by the subroutine, but keep track of the individual locals
     * states that will be expected when the individual subroutine calls return.
     *
     * @param subLabel label of subroutine start block
     * @param callerLabel {@code >=0;} label of the caller block where this
     * frame came from.
    ...
    /**
     * Annotates (adds context to) the given exception with information about
     * this frame.
     *
