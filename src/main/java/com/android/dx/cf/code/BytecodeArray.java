...
public final class BytecodeArray {
    ...
    /**
     * {@code non-null;} constant pool to use when resolving constant pool
     * indices
     */
     ...
     * @param pool {@code non-null;} constant pool to use when resolving
     ...
     * {@code Code} attribute. The returned value includes the array size plus
     * four bytes for {@code code_length}.
     *
    ...
    /**
     * Processes the given "work set" by repeatedly finding the lowest bit in
     * the set, clearing it, and parsing and visiting the instruction at the
     * indicated offset (that is, the bit index), repeating until the work set
     * is empty. It is expected that the visitor will regularly set new bits in
     * the work set during the process.
     *
     * @param workSet {@code non-null;} the work set to process
     * @param visitor {@code non-null;} visitor to call back to for each
     * instruction
     */
    ...
    /**
     * Parses the instruction at the indicated offset. Indicate the result by
     * calling the visitor if supplied and by returning the number of bytes
     * consumed by the instruction.
     *
     * <p>
     * In order to simplify further processing, the opcodes passed to the
     * visitor are canonicalized, altering the opcode to a more universal one
     * and making formerly implicit arguments explicit. In particular:</p>
     *
     * <ul>
     * <li>The opcodes to push literal constants of primitive types all become
     * {@code ldc}. E.g., {@code fconst_0}, {@code sipush}, and {@code lconst_0}
     * qualify for this treatment.</li>
     * <li>{@code aconst_null} becomes {@code ldc} of a "known null."</li>
     * <li>Shorthand local variable accessors become the corresponding longhand.
     * E.g. {@code aload_2} becomes {@code aload}.</li>
     * <li>{@code goto_w} and {@code jsr_w} become {@code goto} and {@code jsr}
     * (respectively).</li>
     * <li>{@code ldc_w} becomes {@code ldc}.</li>
     * <li>{@code tableswitch} becomes {@code lookupswitch}.
     * <li>Arithmetic, array, and value-returning ops are collapsed to the
     * {@code int} variant opcode, with the {@code type} argument set to
     * indicate the actual type. E.g., {@code fadd} becomes {@code iadd}, but
     * {@code type} is passed as {@code Type.FLOAT} in that case. Similarly,
     * {@code areturn} becomes {@code ireturn}. (However, {@code return} remains
     * unchanged.</li>
     * <li>Local variable access ops are collapsed to the {@code int} variant
     * opcode, with the {@code type} argument set to indicate the actual type.
     * E.g., {@code aload} becomes {@code iload}, but {@code type} is passed as
     * {@code Type.OBJECT} in that case.</li>
     * <li>Numeric conversion ops ({@code i2l}, etc.) are left alone to avoid
     * too much confustion, but their {@code type} is the pushed type. E.g.,
     * {@code i2b} gets type {@code Type.INT}, and {@code f2d} gets type
     * {@code Type.DOUBLE}. Other unaltered opcodes also get their pushed type.
     * E.g., {@code arraylength} gets type {@code Type.INT}.</li>
     * </ul>
     *
    ...
    public interface Visitor {
        ...
        /**
         * Visits an instruction which has no inline arguments (implicit or
         * explicit).
         *
         * @param opcode the opcode
         * @param offset offset to the instruction
        ...
        /**
         * Visits an instruction which has a (possibly synthetic) constant
         * argument, and possibly also an additional literal integer argument.
         * In the case of {@code multianewarray}, the argument is the count of
         * dimensions. In the case of {@code invokeinterface}, the argument is
         * the parameter count or'ed with the should-be-zero value left-shifted
         * by 8. In the case of entries of type {@code int}, the {@code value}
         * field always holds the raw value (for convenience of clients).
         *
         * <p>
         * <b>Note:</b> In order to avoid giving it a barely-useful visitor all
         * its own, {@code newarray} also uses this form, passing {@code value}
         * as the array type code and {@code cst} as a {@link CstType} instance
         ...
         * @param value additional literal integer argument, if salient (ignore
         * if not)
         */
         ...
         * @param offset offset to the instruction
         * @param length length of the instruction, in bytes
         * @param cases {@code non-null;} list of (value, target) pairs, plus
         * the default target
         * @param padding the bytes found in the padding area (if any), packed
         */
         ...
         * @param initVals {@code non-null;} list of bytecode offsets for init
         * values
         */
    ...
    /**
     * Base implementation of {@link Visitor}, which has empty method bodies for
     * all methods.
     */
