 ...
 * single instance, they must synchronize access explicitly between
 * themselves.</p>
 */
public class Simulator {
     ...
     * <code>byte[]</code>s, <code>boolean[]</code>s, and known-nulls.</p>
     *
     * <p>
     ...
     * whichever one was actually present on the stack.</p>
     *
     * <p>
     * In the case where there is a known-null on the stack where an array is
     * expected, our behavior depends on the implied type of the instruction.
     * When the implied type is a reference, we don't attempt to infer anything,
     * as we don't know the dimension of the null constant and thus any explicit
     * inferred type could be wrong. When the implied type is a primitive, we
     * fall back to the implied type of the instruction. Due to the quirk
     * described above, this means that source code that uses
     ...
     *
     * @param impliedType {@code non-null;} type implied by the instruction; is
     * <i>not</i> an array type
     * @param foundArrayType {@code non-null;} type found on the stack; is
     * either an array type or a known-null
