...
public final class Ropper {
    ...
    /**
     * Keeps mapping of an input exception handler target code and how it is
     * generated/targeted in Rop.
     */
    private class CatchInfo {

        /**
         * {@code non-null;} map of ExceptionHandlerSetup by the type they
         * handle
         */
        ...
        /**
         * Get the {@link ExceptionHandlerSetup} corresponding to the given
         * type. The ExceptionHandlerSetup is created if this the first request
         * for the given type.
         *
         * @param caughtType {@code non-null;} the type catch by the requested
         * setup
         * @return {@code non-null;} the handler setup block info for the given
         * type
         */
     ...
     * <ul>
     * <li>[0, maxLabel[ are the labels of the blocks directly corresponding to
     * the input bytecode.</li>
     * <li>[maxLabel, maxLabel + method.getCatches().size()[ are reserved for
     * exception setup blocks.</li>
     * <li>[maxLabel + method.getCatches().size(), maxLabel +
     * method.getCatches().size() + SPECIAL_LABEL_COUNT[ are reserved for
     * special blocks, ie param assignement, return and synch blocks.</li>
     * <li>[maxLabel method.getCatches().size() + SPECIAL_LABEL_COUNT,
     * getAvailableLabel()[ assigned labels. Note that some of the assigned
     * labels may not be used any more if they were assigned to a block that was
     * deleted since.</li>
     * </ul>
     *
     * @return {@code >= 0;} an available label with the guaranty that all
     * greater labels are also available.
     */
     ...
     * @param subroutines {@code non-null;} subroutine label list as described
     * in {@link Frame#getSubroutines}
     */
     ...
     * @param pred predecessor label; must be {@code >= 0} when {@code label} is
     * a subroutine start block and calledSubroutine is non-null. Otherwise, may
     * be -1.
    ...
    private class SubroutineInliner {
         ...
         * @param subroutineStart {@code >= 0;} a subroutine as identified by
         * the label of its start block
