...
public class DirectClassFile implements ClassFile {
     ...
     *
     * See http://en.wikipedia.org/wiki/Java_class_file for an up-to-date list
     * of version numbers. Currently known (taken from that table) are:
     *
     * J2SE 7.0 = 51 (0x33 hex), J2SE 6.0 = 50 (0x32 hex), J2SE 5.0 = 49 (0x31
     * hex), JDK 1.4 = 48 (0x30 hex), JDK 1.3 = 47 (0x2F hex), JDK 1.2 = 46
     * (0x2E hex), JDK 1.1 = 45 (0x2D hex).
     *
     * Valid ranges are typically of the form "A.0 through B.C inclusive" where
     * A <= B and C >= 0, which is why we don't have a
     * CLASS_FILE_MIN_MINOR_VERSION.
     */
     ...
     *
     * Note: if you change this, please change "java.class.version" in
     * System.java.
     */
    ...
    /**
     * whether to be strict about parsing; if {@code false}, this avoids doing
     * checks that only exist for purposes of verification (such as magic number
     * matching and path-package consistency checking)
     */
    private final boolean strictParse;

    /**
     * {@code null-ok;} the constant pool; only ever {@code null} before the
     * constant pool is successfully parsed
     */
    private StdConstantPool pool;

    /**
     * the class file field {@code access_flags}; will be {@code -1} before the
     * file is successfully parsed
     */
    private int accessFlags;

    /**
     * {@code null-ok;} the class file field {@code this_class}, interpreted as
     * a type constant; only ever {@code null} before the file is successfully
     * parsed
     */
    private CstType thisClass;

    /**
     * {@code null-ok;} the class file field {@code super_class}, interpreted as
     * a type constant if non-zero
     */
    private CstType superClass;

    /**
     * {@code null-ok;} the class file field {@code interfaces}; only ever
    ...
    /**
     * {@code null-ok;} the class file field {@code attributes}; only ever
     * {@code null} before the file is successfully parsed
     */
    ...
    /**
     * Returns the string form of an object or {@code "(none)"} (rather than
     ...
     * @param filePath {@code non-null;} the file path for the class, excluding
     * any base directory specification
     * @param strictParse whether to be strict about parsing; if {@code false},
     * this avoids doing checks that only exist for purposes of verification
     * (such as magic number matching and path-package consistency checking)
     */
    ...
    /**
     * Constructs and returns an instance of {@link TypeList} whose data comes
     * from the bytes of this instance, interpreted as a list of constant pool
     * indices for classes, which are in turn translated to type constants.
     * Instance construction will fail if any of the (alleged) indices turn out
     * not to refer to constant pool entries of type {@code Class}.
     *
     * @param offset offset into {@link #bytes} for the start of the data
    ...
    /**
     * Gets the class file field {@code magic}, but without doing any checks or
     * parsing first.
     *
    ...
    /**
     * Gets the class file field {@code minor_version}, but without doing any
    ...
    /**
     * Gets the class file field {@code major_version}, but without doing any
     * checks or parsing first.
     *
    ...
    /**
     * Runs {@link #parse} if it has not yet been run to cover up to the
     * interfaces list.
     */
    ...
    /**
     * Implementation of {@link TypeList} whose data comes directly from the
     * bytes of an instance of this (outer) class, interpreted as a list of
     * constant pool indices for classes which are in turn returned as type
     * constants. Instance construction will fail if any of the (alleged)
     * indices turn out not to refer to constant pool entries of type
     * {@code Class}.
     */
