...
public final class RegisterSpec
    ...
    /**
     * {@code null-ok;} local variable info associated with this register, if
     * any
     */
     ...
     * variable info. This method is allowed to return shared instances (but
    ...
    /**
     * Returns an instance for the given register number, type, and variable
     * info. This method is allowed to return shared instances (but doesn't
     * necessarily do so).
     *
     * @param reg {@code >= 0;} the register number
     * @param type {@code non-null;} the type (or possibly actual value) which
    ...
    /**
     * Like {@code equals}, but only consider the simple types of the registers.
     * That is, this compares {@code getType()} on the types to ignore whatever
     * arbitrary extra stuff might be carried around by an outer
     * {@link TypeBearer}.
     *
     * @param other {@code null-ok;} spec to compare to
     * @return {@code true} iff {@code this} and {@code other} are equal in the
     * stated way
     */
     ...
     * This is useful to determine if two instances refer to the "same" local
     * variable.
     *
     * @param other {@code null-ok;} spec to compare to
     * @return {@code true} iff {@code this} and {@code other} are equal in the
    ...
    /**
     * Helper for {@link #equals} and {@link #ForComparison.equals}, which
     ...
     * @return whether this instance is equal to one with the given values
     */
    ...
    /**
     * Compares by (in priority order) register number, unwrapped type (that is
     * types not {@link TypeBearer}s, and local info.
     *
    ...
    /**
     * Helper for {@link #hashCode} and {@link #ForComparison.hashCode}, which
    ...
    /**
     * Gets the type (or actual value) which is loaded from or stored to the
     * register associated with this instance.
     *
     ...
     * This is equal to the register number plus the width (category) of the
     * type used. Among other things, this may also be used to determine the
     * minimum required register count implied by this instance.
     *
    ...
    /**
     * Returns an instance that is the intersection between this instance and
     * the given one, if any. The intersection is defined as follows:
     *
     * <ul>
     * <li>If {@code other} is {@code null}, then the result is {@code null}.
     * <li>If the register numbers don't match, then the intersection is
     * {@code null}. Otherwise, the register number of the intersection is the
     * same as the one in the two instances.</li>
     * <li>If the types returned by {@code getType()} are not {@code equals()},
     * then the intersection is null.</li>
     * <li>If the type bearers returned by {@code getTypeBearer()} are
     * {@code equals()}, then the intersection's type bearer is the one from
     * this instance. Otherwise, the intersection's type bearer is the
     * {@code getType()} of this instance.</li>
     * <li>If the locals are {@code equals()}, then the local info of the
     * intersection is the local info of this instance. Otherwise, the local
     * info of the intersection is {@code null}.</li>
     * </ul>
     *
     * @param other {@code null-ok;} instance to intersect with (or
     * {@code null})
     * @param localPrimary whether local variables are primary to the
     * intersection; if {@code true}, then the only non-null results occur when
     * registers being intersected have equal local infos (or both have
     * {@code null} local infos)
    ...
    /**
     * Returns an instance that is identical to this one, except that the type
    ...
    /**
     * Returns an instance that is identical to this one, except that the type
     * bearer is replaced by the actual underlying type (thereby stripping off
     * non-type information) with any initialization information stripped away
     * as well.
     *
     * @return {@code non-null;} an appropriately-constructed instance
     */
    ...
    /**
     * Returns an instance that is identical to this one except that the local
     * variable is as specified in the parameter.
     *
     ...
     * {@code RegisterSpec} itself can still keep {@code final} instance
     * variables.
     */
    private static class ForComparison {
         ...
         *
         * @param reg {@code >= 0;} the register number
         * @param type {@code non-null;} the type (or possibly actual value)
         * which is loaded from or stored to the indicated register
         * @param local {@code null-ok;} the associated local variable, if any
         * @return {@code non-null;} an appropriately-constructed instance
         */
        ...
        /**
         * Construct a {@code RegisterSpec} of this instance's contents.
         *
         * @return {@code non-null;} an appropriately-constructed instance
         */
