...
/**
 * Set of {@link RegisterSpec} instances, where a given register number may
 * appear only once in the set.
 */
public final class RegisterSpecSet extends MutabilityControl {

    /**
     * {@code non-null;} array of register specs, where each element is
     * {@code null} or is an instance whose {@code reg} matches the array index
     */
     ...
     * is also the maximum-plus-one of register numbers that may be represented.
     *
     ...
     * This is just a convenient shorthand for {@code get(spec.getReg())}.
     *
     ...
     * local (type, name, and signature), or {@code null} if there is none. This
     * ignores the register number of the given spec but matches on everything
     * else.
     *
     ...
     * @return {@code null-ok;} first register found with matching name and
     * signature
     */
    ...
    /**
     * Removes a spec from the set. Only the register number of the parameter is
     * significant.
     *
    ...
    /**
     * Puts the given spec into the set. If there is already an element in the
     * set with the same register number, it is replaced. Additionally, if the
     * previous element is for a category-2 register, then that previous element
     * is nullified. Finally, if the given spec is for a category-2 register,
     * then the immediately subsequent element is nullified.
     *
    ...
    /**
     * Intersects this instance with the given one, modifying this instance. The
     * intersection consists of the pairwise {@link RegisterSpec#intersect} of
     * corresponding elements from this instance and the given one where both
     * are non-null.
     *
     * @param other {@code non-null;} set to intersect with
     * @param localPrimary whether local variables are primary to the
     * intersection; if {@code true}, then the only non-null result elements
     * occur when registers being intersected have equal names (or both have
     * {@code null} names)
     */
