...
/**
 * A register-based instruction. An instruction is the combination of an opcode
 * (which specifies operation and source/result types), a list of actual sources
 * and result registers/values, and additional information.
 */
public abstract class Insn implements ToHuman {
     ...
     * {@inheritDoc}
     *
     * Instances of this class compare by identity. That is, {@code x.equals(y)}
     * is only true if {@code x == y}.
     */
    @Override
     ...
     * {@inheritDoc}
     *
     * This implementation returns the identity hashcode of this instance. This
     * is proper, since instances of this class compare by identity (see
     * {@link #equals}).
     */
    @Override
    ...
    /**
     * Gets an "inline" string portion for toHuman(), if available. This is the
     * portion that appears after the Rop opcode
     *
    ...
    /**
     * Gets the result spec, if any. A return value of {@code null} means this
     * instruction returns nothing.
     *
    ...
    /**
     * Gets whether this instruction can possibly throw an exception. This is
     * just a convenient wrapper for {@code getOpcode().canThrow()}.
     *
     ...
     * StdTypeList#EMPTY} if this instruction has no handlers, which can be
     * <i>either</i> if this instruction can't possibly throw or if it merely
     * doesn't handle any of its possible exceptions. To determine whether this
     * instruction can throw, use {@link #canThrow}.
     *
    ...
    /**
     * Calls the appropriate method on the given visitor, depending on the class
     * of this instance. Subclasses must override this.
     *
    ...
    /**
     * Returns an instance that is just like this one, except that it has a
     * catch list with the given item appended to the end. This method throws an
     * exception if this instance can't possibly throw. To determine whether
     * this instruction can throw, use {@link #canThrow}.
     *
     ...
     * references have been offset by the given delta.
     *
    ...
    /**
     * Returns an instance that is just like this one, except that, if possible,
     * the insn is converted into a version in which a source (if it is a
     * constant) is represented directly rather than as a register reference.
     * {@code this} is returned in cases where the translation is not possible.
     *
     * @return {@code non-null;} an appropriately-constructed instance
     */
    ...
    /**
     * Compares Insn contents, since {@code Insn.equals()} is defined to be an
     * identity compare. Insn's are {@code contentEquals()} if they have the
     * same opcode, registers, source position, and other metadata.
     *
    ...
    /**
     * Returns an instance that is just like this one, except with new result
    ...
    /**
     * Returns the string form of this instance, with the given bit added in the
     * standard location for an inline argument.
     *
    ...
    /**
     * Returns the human string form of this instance, with the given bit added
