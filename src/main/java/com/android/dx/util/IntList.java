...
public final class IntList extends MutabilityControl {
    ...
    /**
     * Adds an element to the end of the list. This will increase the list's
    ...
    /**
     * Inserts element into specified index, moving elements at and above that
     * index up one. May not be used to insert at an index beyond the current
     * size (that is, insertion as a last element is legal but no further).
     *
    ...
    /**
     * Returns the index of the given value, or -1 if the value does not appear
     * in the list. This will do a binary search if the list is sorted or a
     * linear search if not.
     *
    ...
    /**
     * Performs a binary search on a sorted list, returning the index of the
     * given value if it is present or {@code (-(insertion point) - 1)} if the
     * value is not present. If the list is not sorted, then reverts to linear
     * search and returns {@code -size()} if the element is not found.
     *
     * @param value value to find
     * @return index of value or {@code (-(insertion point) - 1)} if the value
     * is not present
     */
    ...
    /**
     * Returns whether or not the given value appears in the list. This will do
     * a binary search if the list is sorted or a linear search if not.
     *
     * @see #sort
     *
