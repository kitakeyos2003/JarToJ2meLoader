...
/**
 * Interface for a binary output destination that may be augmented with textual
 * annotations.
 */
public interface AnnotatedOutput
    ...
    /**
     * Add an annotation for the subsequent output. Any previously open
     * annotation will be closed by this call, and the new annotation marks all
     * subsequent output until another annotation call.
     *
    ...
    /**
     * Add an annotation for a specified amount of subsequent output. Any
     * previously open annotation will be closed by this call. If there is
     * already pending annotation from one or more previous calls to this
     * method, the new call "consumes" output after all the output covered by
     * the previous calls.
     *
     ...
     * output, but annotaters are encouraged to attempt to avoid exceeding the
     * indicated width.
     *
