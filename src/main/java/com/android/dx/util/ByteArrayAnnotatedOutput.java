...
/**
 * Implementation of {@link AnnotatedOutput} which stores the written data into
 * a {@code byte[]}.
 *
 * <p>
 * <b>Note:</b> As per the {@link Output} interface, multi-byte writes all use
...
public final class ByteArrayAnnotatedOutput
    ...
    /**
     * whether the instance is stretchy, that is, whether its array may be
     * resized to increase capacity
     */
    ...
    /**
     * {@code >= 8 (if used);} the number of bytes of hex output to use in
     * annotations
     */
    private int hexCols;

    /**
     * Constructs an instance with a fixed maximum size. Note that the given
     * array is the only one that will be used to store data. In particular, no
     * reallocation will occur in order to expand the capacity of the resulting
     * instance. Also, the constructed instance does not keep annotations by
     * default.
     *
     ...
     * reallocated. The constructed instance does not keep annotations by
     * default.
     */
    ...
    /**
     * Gets the underlying {@code byte[]} of this instance, which may be larger
    ...
    /**
     * Constructs and returns a new {@code byte[]} that contains the written
     * contents exactly (that is, with no extra unwritten bytes at the end).
     *
     * @see #getArray
     *
    ...
    /**
     * Indicates that this instance should keep annotations. This method may be
     * called only once per instance, and only before any data has been written
     * to the it.
     *
     * @param annotationWidth {@code >= 40;} the desired maximum annotation
     * width
    ...
    /**
     * Finishes up annotation processing. This closes off any open annotations
     * and removes annotations that don't refer to written data.
     */
    ...
    /**
     * Throws the excpetion for when an attempt is made to write past the end of
     * the instance.
     */
     ...
     *
     * @param desiredSize {@code >= 0;} the desired minimum total size of the
     * array
     */
