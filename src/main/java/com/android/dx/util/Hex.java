...
public final class Hex {
    ...
    /**
     * Formats an {@code int} as either a 2-byte unsigned hex value (if the
     * value is small enough) or a 4-byte unsigned hex value (if not).
     *
     * @param v value to format
     * @return {@code non-null;} formatted form
     */
    ...
    /**
     * Formats a hex dump of a portion of a {@code byte[]}. The result is always
     * newline-terminated, unless the passed-in length was zero, in which case
     * the result is always the empty string ({@code ""}).
     *
     ...
     * @param addressLength {@code {2,4,6,8};} number of characters for each
     * address header
