...
public final class SsaBasicBlock {
    ...
    /**
     * {@code non-null;} ordered successor list (same block may be listed more
     * than once)
     */
    ...
    /**
     * the number of moves added to the end of the block during the phi-removal
     * process. Retained for subsequent move scheduling.
     */
    ...
    /**
     * contains last computed value of reachability of this block, or -1 if
     * reachability hasn't been calculated yet
     */
    private int reachable = -1;

    /**
     * {@code null-ok;} indexed by reg: the regs that are live-in at this block
     */
    private IntSet liveIn;

    /**
     * {@code null-ok;} indexed by reg: the regs that are live-out at this block
     */
    ...
    /**
     * Adds a phi insn to the beginning of this block. The result type of the
     * phi will be set to void, to indicate that it's currently unknown.
     *
    ...
    /**
     * Adds a phi insn to the beginning of this block. This is to be used when
     * the result type or local-association can be determined at phi insert
     * time.
     *
    ...
    /**
     * Replaces the last insn in this block. The provided insn must have some
     * branchingness.
     *
    ...
    /**
     * @return {@code >= -1;} block index of primary successor or {@code -1} if
    ...
    /**
     * @return {@code null-ok;} the primary successor block or {@code null} if
     * there is none
     */
    ...
    /**
     * Inserts a new empty GOTO block as a predecessor to this block. All
     * previous predecessors will be predecessors to the new block.
     *
    ...
    /**
     * Constructs and inserts a new empty GOTO block {@code Z} between this
     * block ({@code A}) and a current successor block ({@code B}). The new
     * block will replace B as A's successor and A as B's predecessor. A and B
     * will no longer be directly connected. If B is listed as a successor
    ...
    /**
     * Attaches block to an exit block if necessary. If this block is not an
     * exit predecessor or is the exit block, this block does nothing. For use
     * by {@link com.android.dx.ssa.SsaMethod#makeExitBlock}
     *
    ...
    /**
     * Adds a move instruction to the end of this basic block, just before the
     * last instruction. If the result of the final instruction is the source in
     * question, then the move is placed at the beginning of the primary
     * successor block. This is for unversioned registers.
     *
    ...
    /**
     * Checks to see if the register is used in a bitset, taking into account
     * its category/width.
     *
     * @param regsUsed set, indexed by register number
     * @param rs register to mark as used
    ...
    /**
     * Ensures that all move operations in this block occur such that reads of
     * any register happen before writes to that register. NOTE: caller is
     * expected to returnSpareRegisters()!
     *
     * TODO: See Briggs, et al "Practical Improvements to the Construction and
     * Destruction of Static Single Assignment Form" section 5. a) This can be
    ...
    /**
     * Adds {@code regV} to the live-out list for this block. This is called by
    ...
    /**
     * Adds {@code regV} to the live-in list for this block. This is called by
     * the liveness analyzer.
     *
    ...
    /**
     * Returns the set of live-out registers. Valid after register interference
     * graph has been generated, otherwise empty.
     *
    ...
    /**
     * Sorts move instructions added via {@code addMoveToEnd} during phi removal
     * so that results don't overwrite sources that are used. For use after all
     * phis have been removed and all calls to addMoveToEnd() have been made.<p>
     *
     * This is necessary because copy-propogation may have left us in a state
     * where the same basic block has the same register as a phi operand and a
     * result. In this case, the register in the phi operand always refers value
     * before any other phis have executed.
     */
