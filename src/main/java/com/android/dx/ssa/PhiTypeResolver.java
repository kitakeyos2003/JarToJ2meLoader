...
/**
 * Resolves the result types of phi instructions. When phi instructions are
 * inserted, their result types are set to BT_VOID (which is a nonsensical type
 * for a register) but must be resolve to a real type before converting out of
 * SSA form.<p>
 *
 * The resolve is done as an iterative merge of each phi's operand types. Phi
 * operands may be themselves be the result of unresolved phis, and the
 * algorithm tries to find the most-fit type (for example, if every operand is
 * the same constant value or the same local variable info, we want that to be reflected)
 * .<p>
 *
...
public class PhiTypeResolver {
    ...
    /**
     * Returns true if a and b are equal, whether or not either of them are
     * null.
     *
     * @param a
     * @param b
    ...
    /**
     * Resolves the result of a phi insn based on its operands. The "void" type,
     * which is a nonsensical type for a register, is used for registers defined
     * by as-of-yet-unresolved phi operations.
     *
