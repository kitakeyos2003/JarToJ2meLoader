...
public final class SsaMethod {
    ...
    /**
     * Index of exit block, which exists only in SSA form, or or {@code -1} if
     * there is none
     */
    ...
    /**
     * indexed by register: the insn where said register is defined or null if
     * undefined. null until (lazily) created.
     */
    ...
    /**
     * "back-convert mode". Set during back-conversion when registers are about
     * to be mapped into a non-SSA namespace. When true, use and def lists are
     * unavailable.
     *
     ...
     * parameters
     * @param isStatic {@code true} if this method has no {@code this} pointer
     * argument
     */
     ...
     *
     * @param ropMethod {@code non-null;} the original rop-form method that this
    ...
    /**
     * Builds a BitSet of block indices from a basic block list and a list of
     * labels taken from Rop form.
     *
    ...
    /**
     * Builds an IntList of block indices from a basic block list and a list of
     * labels taken from Rop form.
     *
    ...
    /**
     * Creates an exit block and attaches it to the CFG if this method exits.
     * Methods that never exit will not have an exit block. This is called after
     * edge-splitting and phi insertion, since the edges going into the exit
     * block should not be considered in those steps.
     */
     ...
     *
     * @param block block to which this GOTO will be added (not it's
     * destination!)
    ...
    /**
     * Makes a new basic block for this method, which is empty besides a single
     * {@code GOTO}. Successors and predecessors are not yet set.
     *
     * @return new block
     */
    ...
    /**
     * @return {@code null-ok;} block of exit block or {@code null} if there is
     * none
     */
    ...
    /**
     * @return the total width, in register units, of the method's parameters
     */
     ...
     * @return insn (actual instance from code) that defined this reg or null if
     * reg is not defined.
     */
     ...
     * @param oldSources {@code null-ok;} list of sources that were previously
     * used
     */
     ...
     * {@code oldSources} (rather than the sources currently returned by
     * insn.getSources()).
     *
    ...
    /**
     * Adds an insn to both the use and def lists. For use when adding a new
     * insn to the method.
     *
    ...
    /**
     * Removes an instruction from use and def lists. For use during instruction
     * removal.
     *
    ...
    /**
     * Indicates that the instruction list has changed or the SSA register count
     * has increased, so that internal datastructures that rely on it should be
     * rebuild. In general, the various other on* methods should be called in
     * preference when changes occur if they are applicable.
     */
     ...
     *
     * @param insn {@code non-null;} insn who's result should be recorded as a
     * definition
     * @param oldResult {@code null-ok;} a previous result that should be no
     * longer considered a definition by this insn
     */
    ...
    /**
     * Checks to see if the given SSA reg is ever associated with a local local
     * variable. Each SSA reg may be associated with at most one local var.
     *
    ...
    /**
     * Visits blocks in dom-tree order, starting at the current node. The
     * {@code parent} parameter of the Visitor.visitBlock callback is currently
     * always set to null.
     *
    ...
    /**
     * Sets "back-convert mode". Set during back-conversion when registers are
     * about to be mapped into a non-SSA namespace. When true, use and def lists
     * are unavailable.
     */
