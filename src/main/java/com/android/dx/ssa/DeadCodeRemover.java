 ...
 *
 * TODO this algorithm is more efficient if run in reverse from exit block to
 * entry block.
 */
public class DeadCodeRemover {
    ...
    /**
     * indexed by register: whether reg should be examined (does it correspond
     * to a no-side-effect insn?)
     */
     ...
     * @param set a set of registers that we've already determined are only used
     * as sources in operations with no side effect or null if this is the first
     * recursion
    ...
    /**
     * Returns true if this insn has a side-effect. Returns true if the insn is
     * null for reasons stated in the code block.
     *
    ...
    /**
     * A callback class used to build up the initial worklist of registers
     * defined by an instruction with no side effect.
     */
    static private class NoSideEffectVisitor implements SsaInsn.Visitor {
         ...
         *
         * @param noSideEffectRegs to-build bitset of regs that are results of
         * regs with no side effects
         */
