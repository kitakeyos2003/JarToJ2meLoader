 ...
 *
 * This package contains classes associated with dx's {@code SSA} intermediate
 * form. This form is a static-single-assignment representation of Rop-form a
 * method with Rop-form-like instructions (with the addition of a
 ...
 * <li> {@link SCCP} is a (partially implemented) sparse-conditional constant
 * propogator.
 * <li> {@link LiteralOpUpgrader} is a step that attempts to use constant
 * information to convert math and comparison instructions into constant-bearing
 * "literal ops" in cases where they can be represented in the output form (see
 * {@link TranslationAdvice#hasConstantOperation}).
 * <li> {@link ConstCollector} is a step that attempts to trade (modest)
 * increased register space for decreased instruction count in cases where the
 * same constant value is used repeatedly in a single method.
 * <li> {@link DeadCodeRemover} is a dead code remover. This phase must always
 * be run to remove unused phi instructions.
 * </ul>
 *
 * <h2>SSA Lifetime</h2>
 ...
 *
 * {@link SsaConverter#convertToSsaMethod} takes a {@code RopMethod} and returns
 * a fully-converted {@code SsaMethod}. The conversion process is roughly as
 * follows:
 *
 * <ol>
 * <li> The Rop-form method, its blocks and their instructions are directly
 * wrapped in {@code SsaMethod}, {@code SsaBasicBlock} and {@code SsaInsn}
 * instances. Nothing else changes.
 ...
 * <li> A {@link LocalVariableExtractor} is run to produce a table of Rop
 * registers to local variables necessary during phi placement. This step could
 * also be done in Rop form and then updated through the preceding steps.
 ...
 * number (register categories or widths, significant in Rop form, do not exist
 * in SSA). Move instructions are eliminated except where necessary to preserve
