...
/**
 * Searches for basic blocks that all have the same successor and insns but
 * different predecessors. These blocks are then combined into a single block
 * and the now-unused blocks are deleted. These identical blocks frequently are
 * created when catch blocks are edge-split.
 */
public class IdenticalBlockCombiner {
    ...
    /**
     * Runs algorithm. TODO: This is n^2, and could be made linear-ish with a
     * hash. In particular, hash the contents of each block and only compare
     * blocks with the same hash.
     *
    ...
    /**
     * Combines blocks proven identical into one alpha block, re-writing all of
     * the successor links that point to the beta blocks to point to the alpha
     * block instead.
     *
