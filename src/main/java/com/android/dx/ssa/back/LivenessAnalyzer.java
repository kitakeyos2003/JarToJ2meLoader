...
/**
 * From Appel "Modern Compiler Implementation in Java" algorithm 19.17 Calculate
 * the live ranges for register {@code reg}
 * .<p>
 *
 * v = regV
 * <p>
...
public class LivenessAnalyzer {
    ...
    /**
     * {@code non-null;} index by basic block indexed set of basic blocks that
     * have already been visited. "M" as written in the original Appel
     * algorithm.
     */
    ...
    /**
     * {@code >=0;} SSA register currently being analyzed. "v" in the original
     * Appel algorithm.
     */
    ...
    /**
     * Runs register liveness algorithm for a method, updating the live in/out
     * information in {@code SsaBasicBlock} instances and returning an
     * interference graph.
     *
     * @param ssaMeth {@code non-null;} method to process
     * @return {@code non-null;} interference graph indexed by SSA registers in
     * both directions
     */
     ...
     * @param interference {@code non-null;} indexed by SSA reg in both
     * dimensions; graph to update
     *
     */
     ...
     * Obviously, that's not efficient in java, so this function serves as the
     * dispatcher instead.
     */
    ...
    /**
     * Ensures that all the phi result registers for all the phis in the same
     * basic block interfere with each other. This is needed since the dead code
     * remover has allowed through "dead-end phis" whose results are not used
     * except as local assignments. Without this step, a the result of a
     * dead-end phi might be assigned the same register as the result of another
     * phi, and the phi removal move scheduler may generate moves that
     * over-write the live result.
     *
