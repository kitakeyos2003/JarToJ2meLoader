...
public class SsaToRop {
    ...
    /**
     * {@code true} if the converter should attempt to minimize the rop-form
     * register count
     */
     ...
     * @param minimizeRegisters {@code true} if the converter should attempt to
     * minimize the rop-form register count
     ...
     * much of this work will be done later when converting from rop to dex, not
     * all simplification cases can be handled there. Furthermore, any no-op
    ...
    /**
     * See Appel 19.6. To remove the phi instructions in an edge-split SSA
     * representation we know we can always insert a move in a predecessor
     * block.
     */
    ...
    /**
     * Helper for {@link #removePhiFunctions}: PhiSuccessorUpdater for adding
     * move instructions to predecessors based on phi insns.
     */
    ...
    /**
     * Moves the parameter registers, which allocateRegisters() places at the
     * bottom of the frame, up to the top of the frame to match Dalvik calling
     * convention.
     */
    ...
    /**
     * Validates that a basic block is a valid end predecessor. It must end in a
     * RETURN or a THROW. Throws a runtime exception on error.
     *
