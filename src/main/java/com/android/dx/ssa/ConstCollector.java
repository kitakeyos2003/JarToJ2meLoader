...
/**
 * Collects constants that are used more than once at the top of the method
 * block. This increases register usage by about 5% but decreases insn size by
 * about 3%.
 */
public class ConstCollector {
    ...
    /**
     * Also collect string consts, although they can throw exceptions. This is
     * off now because it just doesn't seem to gain a whole lot. TODO if you
     * turn this on, you must change SsaInsn.hasSideEffect() to return false for
     * const-string insns whose exceptions are not caught in the current method.
     */
    ...
    /**
     * Gets all of the collectable constant values used in this method, sorted
     * by most used first. Skips non-collectable consts, such as non-string
     * object constants
     *
    ...
    /**
     * Inserts mark-locals if necessary when changing a register. If the
     * definition of {@code origReg} is associated with a local variable, then
     * insert a mark-local for {@code newReg} just below it. We expect the
     * definition of {@code origReg} to ultimately be removed by the dead code
     * eliminator
     *
