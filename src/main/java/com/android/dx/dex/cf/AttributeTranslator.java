...
/**
 * Utility methods that translate various classfile attributes into forms
 * suitable for use in creating {@code dex} files.
 */
/*package*/ class AttributeTranslator {
    ...
    /**
     * Gets the annotations out of a given {@link AttributeList}. This combines
     * both visible and invisible annotations into a single result set and also
     * adds in a system annotation for the {@code Signature} attribute if
     * present.
     *
     ...
     * #getAnnotations}, also including annotations for translations of
     * class-level attributes {@code EnclosingMethod} and {@code InnerClasses},
     * if present. Additionally, if the class is an annotation class, then this
     * also includes a representation of all the {@code AnnotationDefault}
     * values.
     *
     ...
     * #getAnnotations}, also including an annotation for the translation of the
     * method-specific attribute {@code Exceptions}.
     *
     * @param method {@code non-null;} the method in question
     * @return {@code non-null;} the set of annotations, which may be empty
     */
    ...
    /**
     * Helper method for {@link #getAnnotations} which just gets the existing
     * annotations, per se.
     *
     * @param attribs {@code non-null;} the attributes list to search in
     * @return {@code non-null;} the set of annotations, which may be empty
    ...
    private static Annotations getAnnotations0(AttributeList attribs) {
        AttRuntimeVisibleAnnotations visible
                = (AttRuntimeVisibleAnnotations) attribs.findFirst(AttRuntimeVisibleAnnotations.ATTRIBUTE_NAME);
        AttRuntimeInvisibleAnnotations invisible
                = (AttRuntimeInvisibleAnnotations) attribs.findFirst(AttRuntimeInvisibleAnnotations.ATTRIBUTE_NAME);
     ...
     *
     * @param attribs {@code non-null;} the attributes list to search in
     * @return {@code null-ok;} the converted {@code Signature} annotation, if
     * there was an attribute to translate
     */
     ...
     * {@link AttributeList}, if any, translating it to an annotation. If the
     * class really has an enclosing method, this returns an
     * {@code EnclosingMethod} annotation; if not, this returns an
     * {@code EnclosingClass} annotation.
     *
     * @param attribs {@code non-null;} the attributes list to search in
     * @return {@code null-ok;} the converted {@code EnclosingMethod} or
     * {@code EnclosingClass} annotation, if there was an attribute to translate
     */
     ...
     * {@code InnerClass}, {@code EnclosingClass}, or {@code MemberClasses}
     * annotation.
     *
     * @param thisClass {@code non-null;} type representing the class being
     * processed
     * @param attribs {@code non-null;} the attributes list to search in
     * @param needEnclosingClass whether to include an {@code EnclosingClass}
     * annotation
     * @return {@code null-ok;} the converted list of annotations, if there was
     * an attribute to translate
     */
    ...
    /**
     * Gets the parameter annotations out of a given method. This combines both
     * visible and invisible annotations into a single result set.
     *
     * @param method {@code non-null;} the method in question
    ...
    public static AnnotationsList getParameterAnnotations(Method method) {
        ...
        AttRuntimeVisibleParameterAnnotations visible
                = (AttRuntimeVisibleParameterAnnotations) attribs.findFirst(
                        AttRuntimeVisibleParameterAnnotations.ATTRIBUTE_NAME);
        AttRuntimeInvisibleParameterAnnotations invisible
                = (AttRuntimeInvisibleParameterAnnotations) attribs.findFirst(
    ...
    /**
     * Gets the {@code AnnotationDefault} attributes out of a given class, if
     * any, reforming them as an {@code AnnotationDefault} annotation.
     *
     * @param cf {@code non-null;} the class in question
     * @return {@code null-ok;} an appropriately-constructed
     * {@code AnnotationDefault} annotation, if there were any annotation
     * defaults in the class, or {@code null} if not
     */
