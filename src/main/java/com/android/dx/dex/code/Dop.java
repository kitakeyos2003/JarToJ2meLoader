...
public final class Dop {
    ...
    /**
     * {@code Opcodes.isValid();} what opcode (by number) to try next when
     * attempting to match an opcode to particular arguments;
     * {@code Opcodes.NO_NEXT} to indicate that this is the last opcode to try
     ...
     *
     * @param opcode {@code Opcodes.isValid();} the opcode value itself
     * @param family {@code Opcodes.isValid();} the opcode family
     * @param nextOpcode {@code Opcodes.isValid();} what opcode (by number) to
     * try next when attempting to match an opcode to particular arguments;
     * {@code Opcodes.NO_NEXT} to indicate that this is the last opcode to try
     * in a particular chain
     * @param format {@code non-null;} the instruction format
     * @param hasResult whether the opcode has a result register; if so it is
     * always the first register
     */
    ...
    /**
     * Gets the opcode value to try next when attempting to match an opcode to
     * particular arguments. This returns {@code
     * Opcodes.NO_NEXT} to indicate that this is the last opcode to try in a
     * particular chain.
     *
     * @return {@code Opcodes.MIN_VALUE..Opcodes.MAX_VALUE;} the opcode value
     */
