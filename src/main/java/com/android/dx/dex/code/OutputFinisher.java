...
/**
 * Processor for instruction lists, which takes a "first cut" of instruction
 * selection as a basis and produces a "final cut" in the form of a
 * {@link DalvInsnList} instance.
 */
public final class OutputFinisher {
    ...
    /**
     * {@code >= 0;} the count of reserved registers (low-numbered registers
     * used when expanding instructions that can't be represented simply);
     * becomes valid after a call to {@link
     * #massageInstructions}
     */
    private int reservedCount;

    /**
     * {@code >= 0;} the count of reserved registers just before parameters in
     * order to align them.
     */
     ...
     * @param paramSize size, in register units, of all the parameters for this
     * method
     */
    ...
    /**
     * Returns whether any of the instructions added to this instance come with
     * position info.
     *
     * @return whether any of the instructions added to this instance come with
     * position info
     */
    ...
    /**
     * Helper for {@link #add} which scrutinizes a single instruction for local
     * variable information.
     *
     * @param insn {@code non-null;} instruction to scrutinize
     * @return {@code true} iff the instruction refers to any named locals
     */
    ...
    /**
     * Helper for {@link #hasAnyLocalInfo} which scrutinizes a single register
     * spec.
     *
     * @param spec {@code non-null;} spec to scrutinize
     * @return {@code true} iff the spec refers to any named locals
     */
    ...
    /**
     * Returns the set of all constants referred to by instructions added to
     * this instance.
     *
    ...
    /**
     * Helper for {@link #getAllConstants} which adds all the info for a single
     * instruction.
     *
    ...
    /**
     * Helper for {@link #add}, which updates the position and local info flags.
     *
    ...
    /**
     * Reverses a branch which is buried a given number of instructions backward
     * in the output. It is illegal to call this unless the indicated
     * instruction really is a reversible branch.
     *
     * @param which how many instructions back to find the branch; {@code 0} is
     * the most recently added instruction, {@code 1} is the instruction before
     * that, etc.
    ...
    /**
     * Assigns indices in all instructions that need them, using the given
     * callback to perform lookups. This should be called before calling
     * {@link #finishProcessingAndGetList}.
     *
    ...
    /**
     * Does final processing on this instance and gets the output as a
     * {@link DalvInsnList}. Final processing consists of:
     *
     * <ul>
     * <li>optionally renumbering registers (to make room as needed for expanded
     * instructions)</li>
     * <li>picking a final opcode for each instruction</li>
     * <li>rewriting instructions, because of register number, constant pool
     * index, or branch target size issues</li>
     * <li>assigning final addresses</li>
     * </ul>
     *
     * <p>
     * <b>Note:</b> This method may only be called once per instance of this
     * class.</p>
     *
     * @return {@code non-null;} the output list
     * @throws UnsupportedOperationException if this method has already been
     * called
     */
    ...
    /**
     * Helper for {@link #finishProcessingAndGetList}, which extracts the opcode
     * out of each instruction into a separate array, to be further manipulated
     * as things progress.
     *
    ...
    /**
     * Helper for {@link #finishProcessingAndGetList}, which figures out how
     * many reserved registers are required and then reserving them. It also
     * updates the given {@code opcodes} array so as to avoid extra work when
     * constructing the massaged instruction list.
     *
     * @param opcodes {@code non-null;} array of per-instruction opcode
     * selections
    ...
    /**
     * Helper for {@link #reserveRegisters}, which does one pass over the
     * instructions, calculating the number of registers that need to be
     * reserved. It also updates the {@code opcodes} list to help avoid extra
     * work in future register reservation passes.
     *
     * @param opcodes {@code non-null;} array of per-instruction opcode
     * selections
    ...
    /**
     * Attempts to fit the given instruction into a specific opcode, returning
     * the opcode whose format that the instruction fits into or {@code null} to
     * indicate that the instruction will need to be expanded. This fitting
     * process starts with the given opcode as a first "best guess" and then
     * pessimizes from there if necessary.
     *
     * @param insn {@code non-null;} the instruction in question
     * @param guess {@code null-ok;} the current guess as to the best opcode;
     * {@code null} means that no simple opcode fits
     * @return {@code null-ok;} a possibly-different opcode; either a
     * {@code non-null} good fit or {@code null} to indicate that no simple
     * opcode fits
     */
    ...
    /**
     * Finds the proper opcode for the given instruction, ignoring register
     * constraints.
     *
     * @param insn {@code non-null;} the instruction in question
    ...
    /**
     * Helper for {@link #finishProcessingAndGetList}, which goes through each
     * instruction in the output, making sure its opcode can accomodate its
     * arguments. In cases where the opcode is unable to do so, this replaces
     * the instruction with a larger instruction with identical semantics that
     * <i>will</i> work.
     *
     * <p>
     * This method may also reserve a number of low-numbered registers,
     * renumbering the instructions' original registers, in order to have
     * register space available in which to move very-high registers when
     * expanding instructions into multi-instruction sequences. This expansion
     * is done when no simple instruction format can be found for a given
     * instruction that is able to accomodate that instruction's registers.</p>
     *
     * <p>
     * This method ignores issues of branch target size, since final addresses
     * aren't known at the point that this method is called.</p>
     *
     * @param opcodes {@code non-null;} array of per-instruction opcode
     * selections
     */
    ...
    /**
     * Helper for {@link #massageInstructions}, which constructs a replacement
     * list, where each {link DalvInsn} instance that couldn't be represented
     * simply (due to register representation problems) is expanded into a
     * series of instances that together perform the proper function.
     *
     * @param opcodes {@code non-null;} array of per-instruction opcode
     * selections
    ...
    /**
     * Helper for {@link #finishProcessingAndGetList}, which assigns addresses
     * to each instruction, possibly rewriting branches to fix ones that
     * wouldn't otherwise be able to reach their targets.
     */
    ...
    /**
     * Helper for {@link #assignAddressesAndFixBranches}, which assigns an
     * address to each instruction, in order.
     */
    ...
    /**
     * Helper for {@link #assignAddressesAndFixBranches}, which checks the
     * branch target size requirement of each branch instruction to make sure it
     * fits. For instructions that don't fit, this rewrites them to use a
     * {@code goto} of some sort. In the case of a conditional branch that
     * doesn't fit, the sense of the test is reversed in order to branch around
     * a {@code goto} to the original target.
     *
