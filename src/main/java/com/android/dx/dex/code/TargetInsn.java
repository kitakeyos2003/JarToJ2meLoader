...
public final class TargetInsn extends FixedSizeInsn {
     ...
     * unknown ({@code -1}), and the target is initially {@code null}.
     *
     * @param opcode the opcode; one of the constants from {@link Dops}
     * @param position {@code non-null;} source position
     * @param registers {@code non-null;} register list, including a result
     * register if appropriate (that is, registers may be either ins or outs)
    ...
    /**
     * Returns an instance that is just like this one, except that its opcode
     * has the opposite sense (as a test; e.g. a {@code lt} test becomes a
     * {@code ge}), and its branch target is replaced by the one given, and all
     * set-once values associated with the class (such as its address) are
     * reset.
     *
    ...
    /**
     * Gets the target address of this instruction. This is only valid to call
     * if the target instruction has been assigned an address, and it is merely
     * a convenient shorthand for {@code getTarget().getAddress()}.
     *
    ...
    /**
     * Gets the branch offset of this instruction. This is only valid to call if
     * both this and the target instruction each has been assigned an address,
     ...
     *
     * @return {@code true} if the target offset is known or {@code false} if
     * not
     */
